tingfengF.prototype.fileUtilsF=function(){
	var self2=this;
		/**
		 * 如果有扩展名，返回；没有返回"",若传入路径undefined或者null，则返回原值
		 * 扩展名不包含小点
		 */
		self2.getExtentionName=function(path){
			try{
		 ...
		 * @param path
		 */
		self2.getFileName=function(path){
		 ...
		 * @param path
		 */
		self2.getFileNameNoExtentionName=function(path){
			 if(tingfeng.stringUtils.isEmpty(path))
				 return path;
			path=tingfeng.stringUtils.trim(path, true).replace("\\", "/");
			var index1=path.lastIndexOf("/");
...
tingfengF.prototype.fileUtils=new tingfeng.fileUtilsF();
